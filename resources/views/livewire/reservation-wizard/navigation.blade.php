<div class="pb-4 md:pb-6 lg:pb-8">
    <nav aria-label="Steps" class="bg-white">
        <ol role="list" class="divide-zinc-200 dark:divide-zinc-700 rounded-md border border-zinc-200 dark:border-zinc-700 grid grid-cols-4">

            @foreach($steps as $key => $step)
            
            <li class="relative md:flex md:flex-1 rounded-l-md {{$loop->last?'':'border-r'}} border-zinc-200 dark:border-zinc-700 {{$step->isPrevious()? 'bg-green-50':''}} {{$step->isCurrent()?'ring-2 rounded ring-zinc-900':''}}">
                ...
                <a class="group w-full flex items-center {{$step->isPrevious() || $step->isCurrent() ? 'cursor-pointer' : 'cursor-not-allowed'}}"
                    ...
                    <span class="flex grow items-center lg:px-4 px-2 py-4 text-sm font-medium">
                        ...
                        {{-- <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full
                            {{$step->isCurrent() ? 'border-2 border-zinc-900' : ($step->isPrevious() ? 'bg-green-400 group-hover:bg-green-600' : 'border-2 border-zinc-200 dark:border-zinc-700 group-hover:border-gray-400')}}"> --}}

                            @if ($step->isPrevious())
                                
                                <svg viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 grow md:grow-0 {{$step->isCurrent() ? 'text-zinc-900' : ($step->isPrevious() ? 'text-green-600' : 'text-gray-300 group-hover:text-gray-400')}}">
                            ...
                            @else
                                ...
                                class="w-6 h-6 grow md:grow-0 {{$step->isCurrent() ? 'text-zinc-900' : ($step->isPrevious() ? 'text-green-600' : 'text-gray-300 group-hover:text-gray-400')}}">
                                    {!! $step->icon !!}
                                </svg>
                        ...
                        <span class="ml-2 text-sm font-medium {{$step->isCurrent() ? 'text-zinc-900' : ($step->isPrevious()?'text-green-800':'text-gray-300 group-hover:text-gray-400') }} hidden md:block">
                            {{ $step->label }}
                        </span>
                    </span>
                </a>
            ...
            @endforeach
                ...
                {{-- <li class="relative md:flex md:flex-1">
                    ...
                    <a href="#" class="group flex w-full items-center" x-description="Completed Step">
                        <span class="flex items-center px-6 py-4 text-sm font-medium">
                            <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-zinc-900 group-hover:bg-zinc-900">
                ...
                <li class="relative md:flex md:flex-1">
                    ...
                    <a href="#" class="flex items-center px-6 py-4 text-sm font-medium" aria-current="step" x-description="Current Step">
                        <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border-2 border-zinc-900">
                            <span class="text-zinc-900">02</span>
                        </span>
                        <span class="ml-4 text-sm font-medium text-zinc-900">Application form</span>
                    </a>
                ...
                <li class="relative md:flex md:flex-1">
                    <a href="#" class="group flex items-center" x-description="Upcoming Step">
                        <span class="flex items-center px-6 py-4 text-sm font-medium">
                            <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border-2 border-zinc-200 dark:border-zinc-700 group-hover:border-gray-400">
